...
'''
python main.py --dataset HW --cuda  --upscale_factor 3 --crop_size 256 --batch_size 10 --test_batch_size 10 --epochs 40 --clip 1 --step 20 --lr 1e-2
'''
...
def plot():
    y1 = []
    y2 = []
    with open("epoch_loss.txt", 'r') as f:
        line = f.readline()
        while line:
...
def train(model, criterion, epoch, optimizer, training_data_loader):
    ...
    with open("epoch_loss.txt", 'a') as f:
        f.write(str(epoch_loss / len(training_data_loader)) + '\n') 
...
def validate(model, criterion, validating_data_loader):
    ...
    with open("psnr.txt", 'a') as f:
        f.write(str(avg_psnr / len(validating_data_loader)) + '\n')
